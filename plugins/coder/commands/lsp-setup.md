...
### 5. Bash（npm + apt）
- bash-language-server: Bash 语言服务器（npm）
- shellcheck: Shell 静态分析（apt）
- 安装命令: `npm install -g bash-language-server && sudo apt install shellcheck`
- 验证命令: `bash-language-server --version && shellcheck --version`
